...
export default function RecentAISignals({
  ...
  // If using backend API pagination, do NOT slice again on the client.
  const paginated = shouldUseApi
    ? sortedNormalized
    : sortedNormalized.slice((page - 1) * pageSize, page * pageSize);
  ...
  return (
    <Card className="p-6 bg-gradient-glass backdrop-blur-sm border-border/20">
      ...
      <div className="space-y-3">
        {/* Horizontal scroll container to prevent overflow issues on mid-size screens */}
        <div className="w-full overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-glass" role="region" aria-label="Signal history horizontal scroll">
          <div className="min-w-[820px]">
            {/* Table header (desktop) */}
            <div className="grid grid-cols-[140px_120px_120px_96px_96px_120px] gap-4 px-4 py-2 text-xs text-muted-foreground border-b border-border/10">
            ...
            {loading ? (
              ...
              paginated.map((rec, idx) => (
                <div key={rec.id ?? idx} className="p-3 md:p-2 rounded-lg bg-card/30 border border-border/10 hover:shadow-lg transition-colors">
                  <div className="grid grid-cols-[140px_120px_120px_96px_96px_120px] gap-4 items-center">
                    <div>
